...
- **One sentence when possible, two at most: the problem, then what breaks.** State the defect and its failure ("this won't hold because X"), then stop. A second sentence only when the mechanism is genuinely non-obvious (a subtle race, a data-loss path you can't see from the diff). A dev skims a review comment for the gist, not an essay: plainest words available, short sentences, active voice.
