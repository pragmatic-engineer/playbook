...
#
# Self-contained on purpose: like the bash original it reads its own stdin and
# does not import the shared hook library, so a graph rebuild never depends on
# common.py being importable.
...
def _should_skip() -> bool:
    """Mirror the bash guard: with input, skip unless the write is under
    MEMORY_DIR; with no input at all, always rebuild."""
    ...
    try:
        ...
        file_path = ""
    # Expand a leading ~ so the comparison works (bash: ${file_path/#\~/$HOME}).
    if file_path.startswith("~"):
        home = os.environ.get("HOME") or os.path.expanduser("~")
        file_path = home + file_path[1:]
    if not file_path or not file_path.startswith(MEMORY_DIR):
        return True
    return False
