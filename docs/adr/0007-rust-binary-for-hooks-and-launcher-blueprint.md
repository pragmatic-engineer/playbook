...
- Files:
  ...
  - `shell/gen-shared-settings.py` | edit | `SAFETY_REGEXP` accepts both a bare `playbook hook <name>` and the four guard `.sh` filenames, since across the transition the seed legitimately carries both shapes
...
- Files:
  ...
  - `shell/gen-shared-settings.py` | edit | `SAFETY_REGEXP` drops the `.sh` filename branch, since no hook ships as a path any more
