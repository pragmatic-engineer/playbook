...
- `src/init/guards.rs` copies `self_root/hooks/<name>.sh` for each guard into `claude_home/hooks/`, sets mode 0755 under `cfg(unix)`, and verifies the placed file is executable, reporting `StepStatus::Failed` when it cannot. It runs **before** `wire_hooks`, so `wire` never writes a command for a guard that failed to land. It derives its list from `wire::GUARD_SPECS` filtered on `!ported` rather than a second hardcoded list, so WU-13 flipping `ported` automatically stops it copying. WU-13 deletes the module.
...
**Delivery: two PRs, in this order** (added 2026-08-20; `pr-size-limit-vs-test-heavy-rust` makes one PR breach the 1500-line cap).

1. **PR 1, `feat(init): place everything init names`.** `src/init/guards.rs`, `src/init/system_prompt.rs`, `src/init/run.rs`, `src/init/mod.rs`, `src/init/wire.rs` visibility, `tests/init_guards.rs`. Safe to ship alone and **strictly repairing**: today `init` wires four guard paths and places none, and drops `SYSTEM_PROMPT.md` entirely. After this it places them or fails loudly. It also fixes the pre-existing `precommit-check.sh` gap for `/playbook:setup` users. Nothing downstream depends on the switchover.
2. **PR 2, `feat(install): bootstrap the binary and retire the plugin hook registry`.** Everything else, with the atomic switchover in a single commit so one `git revert` restores `hooks.json`, the old seed and the old `install.sh` together.
