...
**The 4 always-on safety guards** (`rm-workspace-guard`, `bg-await-guard`, `no-dash-guard`, `precommit-check`) are wired directly inside `settings.shared.json`, the template `playbook init` seeds or three-way-merges into a user's `settings.json`. Its `PreToolUse` block already carries all four in final `playbook hook <name>` form, matcher `Bash`, with `rm-workspace-guard` and `precommit-check` each scoped further by an `if` condition (`Bash(rm:*)` and `Bash(git commit:*)`).
...
| `Bash` | `no-dash-guard` | Denies a posting command that carries an em or en dash, in the command text or in a body file it references. Scoped to posting commands, the last chokepoint before prose reaches GitHub or git history. |
