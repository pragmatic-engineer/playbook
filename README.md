...
| 5. Status line | After `install.sh` | Installs `~/.claude/statusline.sh`. `/playbook:setup` does **not** install it; `/playbook:doctor` checks it. `install.sh` also marks `~/.config/playbook` (where the rest of playbook's own state lives) as a trusted workspace in `~/.claude.json`, so a `claude` session started directly in that folder doesn't hit the trust dialog and silently skip the status line; best-effort, needs `jq` or `python3`. |
