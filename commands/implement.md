...
- Invoke the `playbook:engineering-standards` skill (testing requirements, mocking, PR readiness, deployment), the `playbook:grounding-research` skill (verify before asserting), and `playbook:writing-style` (for any prose, e.g. commit messages and the PR body).
