# Golden fixtures: frozen python oracles

Each file here is the exact output of a python hook that ADR 0007 replaced with
a Rust port, captured while that script still existed and committed so the
cross-implementation check survives its deletion.

Before this, several tests were differential: they ran the python original and
